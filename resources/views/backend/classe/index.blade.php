    ...
    <header class="py-4">
        <div class="row">
            <div class="col-sm-6 text-sm-start">
                <div class="container-fluid">
                    <h1 class="h3 fw-normal mb-0">Classes</h1>
                </div>
            </div>
            <div class="col-sm-6 text-sm-end">
                <div class="container-fluid">
                    <a href="{{route('ajout_classe')}}" class="btn btn-secondary">Ajouter une classe</a>
                </div>
            </div>
        </div>
    </header>
            ...
            <div class="card">
                ...
                <div class="table-responsive">
                    <table class="table text-sm mb-0" id="table">
                        ...
                        <tbody>
                            @forelse($classes as $key=>$classe)
                                <tr>
                                    <th scope="row">{{$key + 1}}</th>
                                    <td>{{$classe->nom}}</td>
                                    <td>{{$classe->image}}</td>
                                    <td><a href="{{route('update_classe', ['classe'=>$classe])}}" title="Modifier"><i class="fa fa-edit text-success"></i></a></td>
                                    <td><a href="#" title="Supprimer" id="btn_delete_{{$classe->id}}"><i class="fa fa-trash text-danger"></i></a></td>
                                    <script src="{{asset('backend/vendor/jquerycomfirm/jquery-confirm.min.js')}}"></script>
                                    <script>
                                        $('#btn_delete_{{$classe->id}}').click(function (e){
                                            e.preventDefault();
                                            $.confirm({
                                                title: 'Confirmer?',
                                                type:'green',
                                                ...
                                                buttons: {
                                                    oui: function () {
                                                        ...
                                                        $.ajax({
                                                            url: "/dashboard/classes/delete/{{$classe->id}}",
                                                            type: "DELETE",
                                                            processData: false,
                                                            data:{
                                                                "id": {{$classe->id}},
